;window.CloudflareApps=window.CloudflareApps||{};CloudflareApps.siteId="b70ba21c25ea77024ad8af6134781f22";CloudflareApps.installs=CloudflareApps.installs||{};;(function(){'use strict'
...
return"";}
var _addEventListener=window.addEventListener,_setAttribute=Element.prototype.setAttribute,options=cloneObject(typeof(CloudflareApps.installs['BxdJLaBH5ZdX'].options)!=="undefined"?CloudflareApps.installs['BxdJLaBH5ZdX'].options:{"free":{"guardWhen":"before","guardInnerHTML":"clean","guardPrompt":"clean","guardInput":"clean","guardTransport":"https-only","guardXSS":"sanitize"},"plan":"free"});function isExecutable(str){var functionExp=/([\s\(]*.\(\s*.*\s*\)\s*.*\s*=>)|([\s*.*\s*=>\s*.\{)|([\s\(]*.[Ff]unction\s*.*\s*\(\s*.*\s*\)\s*.*\s*\{)/,templateExp=/(\`(\s*.*)\$\{(\s*.*\s*.*)\}(\s*.*)\`)/;return functionExp.test(str)||templateExp.test(str)};function protect(){if(options.guardInput&&options.guardInput!=="off"){(function(){if(_addEventListener){_addEventListener.call(window,"change",function(event){var target=event.currentTarget;if(target instanceof HTMLInputElement||target instanceof HTMLTextAreaElement){var oldvalue=target.getAttribute("value"),clean=sanitize(target.value);if(oldvalue===clean){return;}
...
if(options.guardWhen==="after"&&_addEventListener){_addEventListener.call(window,"load",protect);}else{protect();}})();};(function(){try{var link=document.createElement('link');link.rel='stylesheet';link.href='data:text/css;charset=utf-8;base64,';document.getElementsByTagName('head')[0].appendChild(link);}catch(e){}})();